...
export default class DebounceLink extends ApolloLink {
    ...
    enqueueRequest(debounceKey: string, { operation, forward, observer }: OperationQueueEntry) {
    ...
    cleanup = (debounceKey: string, groupId: number) => {
    ...
    unsubscribe = (debounceKey: string, debounceGroupId: number, observer: Observer<FetchResult>) => {
